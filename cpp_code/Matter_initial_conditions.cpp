...
void Matter::set_initial_conditions(){
  
  error_handling::general_warning("WARNING in Matter::set_initial_conditions(): initial conditions module still assumes flat universe!");
  error_handling::general_warning("WARNING in Matter::set_initial_conditions(): growth factor is computed for matter density fluctuations and all other components are assumed to be homogeneous!");
  ...
  if(this->cosmology.Omega_r != 0.0){
    ...
    error_handling::general_error_message("ERROR in Matter::set_initial_conditions(): universe has no energy density content besides Lambda!");
  }
  ...
  switch(early_state){
    case RADIATION_DOMINATED:
      if(this->cosmology.Omega_m != 0.0 || this->cosmology.Omega_L != 0.0){
        a_i = min(0.0005*this->cosmology.Omega_r/this->cosmology.Omega_m, pow(0.0005*this->cosmology.Omega_r/this->cosmology.Omega_L, 0.25) ); // go to a time when other components where at most 1/1000 of total energy density
        a_i = min(a_i, this->a_initial);
      }
      this->universe->expansion_in_flat_radiation_dominated_universe(a_i, &t_i, &e_i, &H_i, &H_prime_i);
      D_i = a_i;
      D_prime_i = 0.0;
      break;
    case MATTER_DOMINATED:
      if(this->cosmology.Omega_L != 0.0){
        a_i = 0.0001*pow(this->cosmology.Omega_m/this->cosmology.Omega_L, 1.0/3.0); // go to a time when other components where at most 1/1000 of total energy density
        a_i = min(a_i, this->a_initial);
      }
      this->universe->expansion_in_flat_matter_dominated_universe(a_i, this->cosmology.Omega_m, &t_i, &e_i, &H_i, &H_prime_i);
      D_i = a_i;
      D_prime_i = a_i*H_i;
      break;
