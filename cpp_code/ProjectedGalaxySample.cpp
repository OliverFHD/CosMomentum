...
ProjectedGalaxySample::ProjectedGalaxySample(FlatInhomogeneousUniverseLCDM* universe, double density_in_arcmin_squared, double b1, double b2, double a0, double a1, string input_file) : GalaxySample(universe, b1, b2, a0, a1){
...
double ProjectedGalaxySample::compute_variance_in_angular_tophat(double theta_in_arcmin, double var_NL_rescale){
  return var_NL_rescale*this->pointer_to_universe()->return_LOS_integrated_variance(theta_in_arcmin*constants::arcmin, this->z_values, this->n_of_z_values);
}
...
vector<double> ProjectedGalaxySample::return_CiC_PDF_in_angular_tophat(double theta_in_arcmin, double f_NL, double var_NL_rescale){
  ...
  vector<vector<double> > PDF_data = this->pointer_to_universe()->compute_LOS_projected_PDF(this->z_values, this->n_of_z_values, theta_in_arcmin*constants::arcmin, f_NL, var_NL_rescale);
