...
extern "C" void return_PDF(double* delta_values, double* PDF, int n_delta, double delta_min, double delta_max, double z, double R_in_comoving_Mpc, double f_NL, double var_NL_rescale, int index_of_universe){
  ...
  phi_data = global_universes.universes[index_of_universe]->compute_phi_of_lambda_3D(z, R_in_comoving_Mpc/constants::c_over_e5, f_NL, var_NL_rescale);
  ...
  if(R_max > exp(global_universes.universes[index_of_universe]->log_top_hat_radii_for_skewnesses[global_universes.universes[index_of_universe]->log_top_hat_radii_for_skewnesses.size()-1])*constants::c_over_e5){
    tau_max = min(tau_max, interpolate_neville_aitken(exp(global_universes.universes[index_of_universe]->log_top_hat_radii_for_skewnesses[global_universes.universes[index_of_universe]->log_top_hat_radii_for_skewnesses.size()-1])*constants::c_over_e5, &R_L_values, &tau_values, constants::order_of_interpolation));
  }
  
  if(R_min < exp(global_universes.universes[index_of_universe]->log_top_hat_radii_for_skewnesses[0])*constants::c_over_e5){
    tau_min = max(tau_min, interpolate_neville_aitken(exp(global_universes.universes[index_of_universe]->log_top_hat_radii_for_skewnesses[0])*constants::c_over_e5, &R_L_values, &tau_values, constants::order_of_interpolation));
  }
